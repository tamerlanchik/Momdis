...
%fprintf(fail,'%s\n','global flag_nd;');
%fprintf(fail,'%s\n','if flag_nd==0;');
% хитрый ход, т.к. нальз€ писать символ '. '=char(39)
...
%fprintf(fail,'%s\n','[tt,xx]=ode23s(@mat_mod,[t0 T],X0,options,Q);');
%fprintf(fail,'%s\n','X_=[tt,xx];');
%fprintf(fail,'%s\n','end;');
%fprintf(fail,'%s\n','if flag_nd==1;');
%fprintf(fail,'%s\n','X_=[0,X0];');
...
%fprintf(fail,'%s\n','u_=[];');
%fprintf(fail,'v_=[];');
% ќграничени€ типа неравенств: u_(t)<=0;
% Ќапример:
% u_(1)=Q(1)+Q(2)-1;
% u_(2)=Q(3)+Q(4)-1;
% ќграничени€ типа равенств: v_(t)=0;
fail=fopen(s10,'a');
...
% –азмерности вектора параметров по каждой коалиции
fail=fopen(s10,'a');
